...
class GoodFET:
    ...
    def f(self, x): return str(int(bin(x)[:-3], 2)) + (' %s' % 'w' if int(bin(x)[-2]) == 1 else ' r')
    ...
    def writecmd(self, app, verb, count=0, data=[]):
        ...
        #if self.verbose:
        if 0:
            #print("Tx: ( 0x%02x, 0x%02x, 0x%04x, %s)" % ( app, verb, count, data))
            if type(data) == str:
              data = [ord(x) for x in data]

            extra = ""
            try:
              extra = " ("+self.f(data[0])+") - " + bin(data[1])[2:].zfill(8)
            except:
              pass
            print("Tx: %s%s" % (bytes_as_hex([app]+[verb]+[count]+data), extra))
                ...
                if self.app==0xFF:
                    ...
                    if 0:
                      if type(self.data) == str:
                        data = [ord(x) for x in self.data]
                      print("Rx: %s" % bytes_as_hex([self.app]+[self.verb]+[self.count]+data))

                      #print("Rx: ( 0x%02x, 0x%02x, 0x%04x, %s )" % ( self.app, self.verb, self.count, map(ord, self.data) ))
                    return self.data
            except TypeError:
                if self.connected:
